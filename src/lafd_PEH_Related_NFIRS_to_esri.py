"""
social-and-human-services-lake.uhrc.lafd_PEH_Related_NFIRS to ESRI 
...
table = conn.table('lafd_PEH_Related_NFIRS')
...
def clean_data(df, file):
    # Fix dtypes
    df = df.assign(
        Incident_Date = pandas.to_datetime(df.Incident_Date, errors="coerce"),
    )
